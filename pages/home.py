...
from services.github_service import extract_repo_name, validate_repository_url
...
def analyze():
    ...
    if input_mode == "url":
        ...
        if not filename.lower().endswith((".txt", ".log")):
            flash("Upload a plain-text .txt or .log file.", "error")
            return redirect(url_for("home.index"))
        ...
        repo_name = filename.rsplit(".", maxsplit=1)[0].replace("-", " ").title()
