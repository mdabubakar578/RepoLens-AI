...
TECH_SIGNATURES: dict[str, dict] = {
    ...
    "Django": {
        "files": ["manage.py"],
        "content": ['"django"', "from django", "DJANGO_SETTINGS"],
    },
    ...
    "Kubernetes": {
        ...
        "content": ["apiVersion:", "kind: Deployment"],
    },
...
def analyze_repository(
    ...
    if commits:
        ...
        analysis.risk_items = detect_risks(
            commits, file_tree, analysis.todos, file_contents, analysis.hotspots
        )
...
def detect_technologies(
    file_tree: list[dict], file_contents: dict[str, str]
) -> list[TechDetection]:
        ...
        for sf in sig_files:
            if sf.endswith("/"):
                if any(p.startswith(sf) for p in file_paths):
                    evidence.append(f"Directory: {sf}")
                    score += 0.5
            elif sf in file_names:
                evidence.append(f"File: {sf}")
                score += 0.4
        for pattern in sigs.get("content", []):
            if pattern.lower() in all_content:
                evidence.append(f"Content match: {pattern}")
                score += 0.3
        for ext in sigs.get("extensions", []):
            if extensions.get(ext, 0) > 0:
                evidence.append(f"Extension: {ext} ({extensions[ext]} files)")
                score += 0.2
        if evidence:
            detected.append(
                TechDetection(
...
def compute_language_stats(
    file_tree: list[dict], github_languages: dict[str, int] | None = None
) -> dict[str, float]:
...
def summarize_directory_structure(file_tree: list[dict]) -> str:
    ...
    root_files = [
        item["path"] for item in file_tree if "/" not in item["path"] and item["type"] == "blob"
    ]
    lines.extend(f"📄 {filename}" for filename in sorted(root_files))
                ...
                todos.append(
                    {
                        ...
                        "priority": "high" if m.group(1).upper() in ("FIXME", "HACK") else "normal",
                    }
                )
    return todos[:100]
    ...
    return [
        {
            ...
            "risk": "CRITICAL"
            if count > 15 and len(file_authors[f]) > 2
            else "HIGH"
            if count > 10
            else "MEDIUM"
            if count > 5
        ...
        for f, count in file_mentions.most_common(15)
        if count >= 2
    ]
...
def score_commit_quality(commits: list[dict]) -> dict:
    ...
    has_type = sum(
        ...
        for c in commits
        if re.match(
            r"^(feat|fix|docs|style|refactor|test|chore|ci|perf)\b",
            c.get("message", ""),
            re.IGNORECASE,
        )
    )
    quality_score = max(
        ...
        min(
            100,
            100 - (noisy / total * 30) - (short_msg / total * 30) + (has_type / total * 20),
        ),
    )
    return {
...
def detect_risks(
        ...
        risks.append(
            {
                "type": "tech_debt",
                "severity": "HIGH",
                "confidence": min(1.0, round(0.7 + (len(todos) / 100), 2)),
                "title": f"{len(todos)} TODO/FIXME items found",
                "description": "High number of unresolved tasks may indicate technical debt.",
            }
        )
        ...
        risks.append(
            {
                ...
                "severity": "MEDIUM",
                "confidence": 0.95,
                "title": f"{len(large_files)} large files detected",
                "description": "Files over 50KB are harder to maintain and should be refactored.",
            }
        )
    if commits:
    ...
    code_files = [
        p for p in file_contents if p.endswith((".py", ".js", ".ts", ".java", ".go", ".rs"))
    ]
    if code_files:
        ...
        risks.append(
            {
                "type": "architecture",
                "severity": "CRITICAL",
                "title": "Highly Coupled Files Detected",
...
def compute_complexity_metrics(
    ...
    total_score = round(
        min(100.0, volume_score + depth_score + dep_score + hotspot_score + todo_score)
    )
...
def detect_entry_points(file_tree: list[dict]) -> list[str]:
    ...
    return [
        f["path"]
        for f in file_tree
        if f["type"] == "blob" and os.path.basename(f["path"]).lower() in candidates
    ]
